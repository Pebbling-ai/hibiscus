...
from app.core.auth import Auth, get_current_user_from_api_key
...
async def test_generate_api_key():
    ...
    with mock.patch("app.db.client.Database.create_api_key", mock_create_api_key):
        ...
        # Test without expiry
        result = await Auth.generate_api_key(user_id=user_id, name=name)
...
async def test_get_current_user_from_api_key():
    ...
    user_id = str(uuid.uuid4())
    mock_user = {"id": user_id, "email": "test@example.com", "full_name": "Test User"}
