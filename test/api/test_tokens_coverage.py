...
async def test_create_api_token_with_expiry_days(monkeypatch):
    ...
    async def mock_create_api_key(user_id, name, expires_at=None, description=None):
        ...
        if api_key_data.expires_in_days:
            expected_expiry = datetime.now(timezone.utc) + timedelta(days=api_key_data.expires_in_days)
        ...
        assert abs((expiry_dt - datetime.now(timezone.utc)).total_seconds()) < 30 * 24 * 60 * 60 + 60
...
async def test_create_api_token_permanent(monkeypatch):
    ...
    api_key_data = ApiKeyCreate(
        ...
        description="Token that never expires"
    )
...
async def test_create_api_token_with_absolute_expiry(monkeypatch):
    ...
    with mock.patch('app.db.client.Database.create_api_key', mock_create_api_key):        
        ...
        assert result.expires_at is not None  # We can't check exact value due to timing differences
...
async def test_list_api_tokens_pagination(monkeypatch):
    ...
    for i in range(25):  # 25 tokens to test pagination
        mock_tokens.append({
            ...
            "key": f"test_key_{i}",
            "created_at": datetime.now(timezone.utc).isoformat(),
            "expires_at": (datetime.now(timezone.utc) + timedelta(days=30)).isoformat() if i % 2 == 0 else None,
    ...
    # Apply mocks
    with mock.patch('app.db.client.Database.count_api_keys', mock_count_api_keys), \
