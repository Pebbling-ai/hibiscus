...
def mock_federated_router(test_app, mock_database, mock_auth_dependency):
    ...
    from app.models.schemas import FederatedRegistry, PaginatedResponse
    ...
    async def list_federated_registry_agents(
        ...
        try:
            ...
            agents = await Database.list_agents(
                limit=size, offset=offset, registry_id=registry_id
            )
...
@pytest.mark.asyncio
async def test_sync_federated_registry(
    test_app, client, mock_federated_router, mock_database
):
...
@pytest.mark.asyncio
async def test_sync_federated_registry_not_found(
    test_app, client, mock_federated_router
):
...
@pytest.mark.asyncio
async def test_list_federated_registry_agents(
    test_app, client, mock_federated_router, mock_database
):
    # Create a test registry
    registry = await mock_database.add_federated_registry(
...
async def test_sync_registry_agents_with_pagination(mock_database, monkeypatch):
    ...
    mock_agents = [
        ...
        for i in range(10)
    ]  # 10 agents in one response

    # Mock response with all agents in one page
    mock_response = {
        "items": mock_agents,
        "metadata": {"total": 10, "page": 1, "page_size": 10, "total_pages": 1},
    }
    ...
    with (
        mock.patch(
            "app.api.routes.federated_registries.httpx.AsyncClient",
        ...
        mock.patch(
            ...
            mock_update_registry_sync_time,
        ),
    ):
        ...
        assert len(create_agent_calls) == 10, (
            f"Expected 10 agents, got {len(create_agent_calls)}"
        )
...
async def test_sync_registry_agents_with_verification_data(mock_database, monkeypatch):
    ...
    mock_response_data = {
        ...
        "metadata": {"total": 1, "page": 1, "page_size": 10, "total_pages": 1},
    }
    ...
    async def mock_create_federated_agent(agent_data):
        ...
        data_without_verification = {
            k: v for k, v in agent_data.items() if k != "verification"
        }
    ...
    with (
        ...
        mock.patch(
            "app.db.client.Database.create_agent_verification",
            mock_create_agent_verification,
        ),
        mock.patch(
            "app.db.client.Database.update_federated_registry_sync_time",
            mock.AsyncMock(),
...
async def test_sync_registry_failure_handling(mock_database, monkeypatch):
    ...
    with (
        ...
        assert any("Error synchronizing" in str(args) for args in print_calls), (
