...
if not supabase_url or not supabase_key:
    console.print(
        "[bold red]Error: Missing Supabase credentials. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env file.[/bold red]"
    )
    exit(1)
...
async def create_initial_admin(email: str, name: str, expire_days: int = 365) -> str:
    ...
    with Progress(
        ...
        overall_task = progress.add_task(
            "[yellow]Creating admin account...", total=3, status="Starting"
        )
        user_task = progress.add_task(
            "[green]Creating user record", total=1, status="Pending"
        )
        key_task = progress.add_task(
            "[green]Creating API key", total=1, status="Pending"
        )
