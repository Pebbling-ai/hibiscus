...
class TypesenseClient:
    ...
    async def initialize_collections(cls) -> bool:
        ...
        if not client:
            logger.warning("Typesense client not initialized. Cannot create collections.")
            return False
    ...
    async def update_agent(cls, agent_id: str, agent_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Update document
            response = client.collections[AGENTS_COLLECTION].documents[agent_id].update(document)
    ...
    async def delete_agent(cls, agent_id: str) -> bool:
        ...
        try:
            ...
            search_results = client.collections[AGENTS_COLLECTION].documents.search(search_params)
            ...
            if search_results.get('found', 0) > 0:
                ...
                for hit in hits:
                    ...
                    document_id = document.get('agent_id')  # Use agent_id as the identifier
                    if document_id:
                        # Delete document by ID
                        client.collections[AGENTS_COLLECTION].documents[document_id].delete()
    ...
    @classmethod
    async def search_agents(cls, query: str, page: int = 1, per_page: int = 20, 
        ...
        try:
            ...
            # Execute search
            results = client.collections[AGENTS_COLLECTION].documents.search(search_parameters)
    ...
    async def index_agent_batch(cls, agents: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            response = client.collections[AGENTS_COLLECTION].documents.import_(documents, {'action': 'upsert'})
            
            # Check for errors
            if isinstance(response, list):
                success_count = sum(1 for item in response if not item.get('error'))
                logger.info(f"Indexed {success_count}/{len(documents)} agents in Typesense")
    ...
    @classmethod
    async def bulk_sync_agents(cls, agent_ids: List[str], fetch_agent_fn) -> Dict[str, bool]:
        """
        ...
        if already_exists_count > 0:
            logger.info(f"Skipped {already_exists_count} agents that already exist in Typesense")
    ...
    async def check_agent_exists(cls, agent_id: str) -> bool:
        ...
        client = cls.get_client()
        if not client:
            logger.warning("Typesense client not initialized. Cannot check agent existence.")
            return False
        ...
        try:
            ...
            results = client.collections[AGENTS_COLLECTION].documents.search(search_params)
        ...
        if "created_at" in agent and agent["created_at"]:
            if isinstance(agent["created_at"], str):
                try:
                    ...
                    dt = datetime.fromisoformat(agent["created_at"].replace('Z', '+00:00'))
        ...
        if "updated_at" in agent and agent["updated_at"]:
            if isinstance(agent["updated_at"], str):
                try:
                    # Convert ISO format string to unix timestamp (seconds)
                    from datetime import datetime
                    dt = datetime.fromisoformat(agent["updated_at"].replace('Z', '+00:00'))
