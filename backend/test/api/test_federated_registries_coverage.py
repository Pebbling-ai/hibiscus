...
async def test_list_federated_registries_pagination(monkeypatch):
    ...
    mock_registries = [{
        ...
        "last_synced_at": None
    } for i in range(1, 26)]  # 25 registries
    ...
    # Apply mocks
    with mock.patch('app.db.client.Database.count_federated_registries', mock_count_registries), \
         mock.patch('app.db.client.Database.list_federated_registries', mock_list_registries):
...
async def test_list_federated_registry_agents_pagination(monkeypatch):
    ...
    async def mock_list_agents(limit=20, offset=0, search_term=None, is_team=None, registry_id=None):
...
async def test_sync_registry_agents_with_did_verification(monkeypatch):
    ...
    with mock.patch('httpx.AsyncClient', return_value=MockHTTPClient()), \
         ...
         mock.patch('app.db.client.Database.update_federated_registry_sync_time', mock_update_sync_time), \
         mock.patch('app.db.client.Database.create_agent_verification', mock_create_verification):
