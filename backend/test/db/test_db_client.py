...
from app.db.client import Database, AGENTS_TABLE, AGENT_VERIFICATION_TABLE, API_KEYS_TABLE, FEDERATED_REGISTRIES_TABLE, AGENT_HEALTH_TABLE, USERS_TABLE
...
class TestDatabaseClient:
    ...
    async def test_list_agents(self, setup_supabase):
        ...
        with patch.object(Database, '_parse_agent_json_fields', side_effect=parse_and_merge_side_effect):
            # Test the function
    ...
    async def test_get_agent(self, setup_supabase):
        ...
        with patch.object(Database, '_parse_agent_json_fields', side_effect=parse_side_effect):
            # Test the function
    ...
    async def test_update_agent(self, setup_supabase):
        ...
        update_data = {
            ...
            "capabilities": [{"name": "new_capability", "description": "New capability"}],
            "tags": ["updated", "new"]
        }
    ...
    async def test_create_agent_verification(self, setup_supabase):
        ...
        verification_data = {
            ...
            "did_document": json.dumps({
                "id": f"did:mlts:{uuid.uuid4()}",
                "verificationMethod": [{"id": "#key-1", "type": "Ed25519VerificationKey2018"}]
            })
        }
    ...
    async def test_get_agent_health_summary(self, setup_supabase):
        ...
        setup_supabase.table.side_effect = lambda table_name: table_mocks.get(table_name, MagicMock())
    ...
    async def test_get_agent_with_verification_data(self, setup_supabase):
        ...
        with patch('app.db.client.Database._parse_agent_json_fields', side_effect=lambda x: x):
            # Setup mock agent
            agent_id = str(uuid.uuid4())
            mock_agent = {
                "id": agent_id, 
                ...
                "capabilities": json.dumps([{"name": "secure_comms", "description": "Secure communication"}]),
            ...
            async def mock_get_agent(agent_id):
                ...
                result_agent["did_document"] = json.loads(verification_data["did_document"])
                return result_agent
    ...
    async def test_list_agents_with_teams(self, setup_supabase):
        ...
        with patch('app.db.client.Database.list_agents', mock_list_agents):
            ...
            team_results, team_count = await Database.list_agents(limit=10, offset=0, is_team=True)
            ...
            agent_results, agent_count = await Database.list_agents(limit=10, offset=0, is_team=False)
