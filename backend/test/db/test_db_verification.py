...
from app.db.client import Database, AGENTS_TABLE, AGENT_VERIFICATION_TABLE, FEDERATED_REGISTRIES_TABLE
...
class TestDatabaseVerification:
    ...
    async def test_update_federated_registry_sync_time(self, setup_supabase):
        ...
        setup_supabase.table.return_value.update.return_value.eq.assert_called_once_with("id", registry_id)
    ...
    async def test_count_agents_with_registry_filter(self, setup_supabase):
        ...
        setup_supabase.table.return_value.select.assert_called_once_with("count", count="exact")
    ...
    async def test_count_agents_without_filter(self, setup_supabase):
        ...
        # Setup Supabase mock
        setup_supabase.table.return_value.select.return_value.execute.return_value = mock_response
    ...
    async def test_create_federated_agent(self, setup_supabase):
        ...
        with patch('app.db.client.Database._parse_agent_json_fields', side_effect=lambda x: x):
            ...
            assert result["is_federated"] == True
    ...
    async def test_update_federated_agent(self, setup_supabase):
        ...
        agent_data = {
            ...
            "capabilities": [{"name": "updated_test", "description": "Updated testing capability"}],
        ...
        response_data = {
            ...
            "name": "Updated Federated Agent",
            "description": "An updated federated test agent",
            "capabilities": agent_data["capabilities"],  # No JSON conversion needed with our mock
        ...
        with patch('app.db.client.Database._parse_agent_json_fields', side_effect=lambda x: x):
            ...
            assert agent_data["federation_id"] == original_data["id"]  # And set federation_id
    ...
    async def test_get_agent_by_federation_id(self, setup_supabase):
        ...
        with patch('app.db.client.Database._parse_agent_json_fields', side_effect=lambda x: x):
            ...
            # Chain the mocks
            setup_supabase.table.return_value.select.return_value.eq.return_value = mock_eq_chain
