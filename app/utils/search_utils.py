...
async def create_agent_with_verification(
    ...
    try:
        ...
        if not typesense_record_created:
            logger.error(f"Failed to create agent record in Typesense for agent ID: {created_agent['id']}")
...
async def update_agent_with_typesense(
    ...
    try:
        ...
        typesense_update_needed = any(field in update_data for field in typesense_fields)
        
        # Update in Typesense if relevant fields changed
        if typesense_update_needed:
            try:
                # Update the agent in Typesense
                typesense_success = await TypesenseClient.update_agent(agent_id, updated_agent)
