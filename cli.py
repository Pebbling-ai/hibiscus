...
def list_agents_cmd(
    ...
    for agent in agents:
        ...
        table.add_row(
            ...
            f"{health_emoji} {health_status.capitalize()}"
            if health_status
            else "❓ Unknown",
        )
    ...
    console.print(
        Panel(pagination_text, box=ROUNDED, border_style="blue", expand=False)
    )
...
def update_agent_cmd(
    ...
    console.print(
        Panel(
            "You are about to update the following agent:",
    ...
    if Confirm.ask("Update description?", default=False):
        new_description = Prompt.ask(
            "Enter new description", default=agent.get("description", "")
        )
    ...
    if Confirm.ask("Update domains?", default=False):
        ...
        new_domains_str = Prompt.ask(
            "Enter domains (comma-separated)", default=current_domains
        )
    ...
    if agent.get("metadata") and Confirm.ask("Update metadata?", default=False):
        console.print(
            "[yellow]Note: Manual metadata editing not supported in CLI. Use the API for complex metadata updates.[/]"
        )
    ...
    if agent.get("is_team") and Confirm.ask("Update team mode?", default=False):
        ...
        current_mode_index = (
            modes.index(agent.get("mode", "collaborate"))
            if agent.get("mode") in modes
            else 0
        )
        mode_options = "\n".join([f"{i + 1}. {mode}" for i, mode in enumerate(modes)])
        console.print(f"Team modes:\n{mode_options}")
        mode_choice = Prompt.ask(
            "Select mode (1-3)", default=str(current_mode_index + 1)
        )
        try:
    ...
    async def update_agent_async():
        try:
            ...
            updated_agent = await update_agent_with_typesense(
                agent_id=agent_id, update_data=update_data, current_user_id=user_id
            )
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn("[bold green]Updating agent..."),
        console=console,
        transient=True,
    ) as progress:
    ...
    console.print(
        Panel(
            "Agent updated successfully!", title="Update Complete", border_style="green"
        )
    )
...
def display_agent_details(agent: Dict[str, Any]):
    ...
    if agent.get("domains"):
        domains_str = ", ".join(
            [f"[green]{domain}[/]" for domain in agent.get("domains", [])]
        )
        details.append("")
